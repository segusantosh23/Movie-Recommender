...
const Liked: React.FC = () => {
  ...
  useEffect(() => {
    setSortedMovies(sortMovies(likedMovies, sortOption));
  }, [likedMovies, sortOption]);

  return (
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <h1 className="text-3xl md:text-4xl font-bold border-l-4 border-blue-500 pl-4 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Your Liked Movies</h1>
      ...
      {sortedMovies.length > 0 ? (
        ...
        <p className="text-center text-slate-500 dark:text-slate-400 text-lg mt-10">You haven't liked any movies yet.</p>
      )}
    </div>
  );
};
